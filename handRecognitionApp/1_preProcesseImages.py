...
dir_paths = os.listdir('./data/asl_alphabet_train/asl_alphabet_train')
dir_paths.sort()
...
for idx, dir_path  in tqdm(enumerate(dir_paths), total=len(dir_paths)):
    ...
    os.makedirs(f"./data/preprocessed_images/{dir_path}", exist_ok=True)
    for i in range(args['num_images']): # how many images to preprocess for each category
        ...
        cv2.imwrite(f"./data/preprocessed_images/{dir_path}/{dir_path}{i}.jpg", image)
print('DONE')